...
Either way, it polls every 5–10 minutes by default (randomized, to avoid a
bot-like fixed cadence), and the instant it detects the opening it fires a
...
- ✅ Input is validated as you type — it won't let you proceed with a malformed date or empty required field
...
it as a default — just press Enter if it looks right. The trailing date in
...
- **Fill it in** to wait for one specific theatre instead. Every BMS cinema
  ...
  e.g. `.../pvr-palazzo-the-nexus-vijaya-mall/PVPZ` → the code is `PVPZ`.
...
Defaults to 5–10 minutes, randomized each cycle. Press Enter twice to accept
the defaults, or tighten it for a faster alert — though very frequent
polling increases the chance BMS rate-limits or blocks your IP, see
...
Since the script is now interactive, a plain systemd service has no terminal
...
non-interactive variant that reads a JSON/env config instead — happy to help
build that if you want a true unattended setup later.
...
| Never detects the opening (any-theatre mode) | Double check the date you confirmed matches what BMS actually uses — occasionally date formats shift between events, re-check page source for `date-disabled` |
